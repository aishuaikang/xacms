...
type DroneTargetModel struct {
	...
	DeviceID      uuid.UUID     `json:"device_id" gorm:"type:char(36);index;not null;comment:捕获设备"` // 捕获设备，普通索引
	ParseID       int           `json:"parse_id" gorm:"size:64;not null;comment:捕获设备"`              // 捕获设备
